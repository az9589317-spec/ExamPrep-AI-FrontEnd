...
export default async function UpscCategoryPage() {
    ...
    const upscCategoryData = examCountByCategory['UPSC'] || {};
    const previousYearPaperCount = upscCategoryData['Previous Year Paper'] || 0;
            ...
            <main className="flex-1 p-4 md:p-8">
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {upscSubCategories.map((category) => (
                        <Link href={`/exams/UPSC/${encodeURIComponent(category.name)}`} key={category.name}>
                                ...
                                <CardContent>
                                    <div className="flex items-center justify-between text-sm">
                                        <div className="text-sm font-bold text-muted-foreground bg-muted px-2 py-1 rounded-md">
                                            {upscCategoryData[category.name] || 0} Exams
                                        </div>
                    ...
                    <Link href={`/exams/Previous Year Paper/UPSC`}>
                        <Card className="flex flex-col justify-between h-full hover:bg-card/70 transition-all duration-300 shadow-glow-br hover:shadow-glow-tl">
                            <CardHeader>
                                <div className="flex items-center gap-4">
